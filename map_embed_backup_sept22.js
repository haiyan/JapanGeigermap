...
function gm_final_load() {
    if (gm_js_loaded && gm_maps_loaded) {
        for (var i = 0; i < gm_map_options.length; i++) {
            ...
            //alert("gm_final_load " + gm_map_options[i].map_name);
...
function create_geigermap(options_obj) {
    ...
    **/
        ...
        gm_map_labels[options_obj.map_name] = new Label({
            map: map,
            position: new google.maps.LatLng(38.289937,133.560791),
            clickable: false,
            text: "Loading geiger counter feeds..."
        });
...
function gm_process_json(oJson) {
	//alert("gm_process_json " + oJson.results.length);
    for (var i = 0; i < oJson.results.length; i++) { //
        ...
        //alert("gm_process_json " + curr_feed.title);
...
function gm_draw_label(thismap, reading, loc, content, title, loc_name, unit, id, stream_id) {
	//alert("gm_draw_labl " + reading + " loc: " + loc + " content: " + content);
    ...
    label.div_.style.cssText = alertdiv + "z-index:" + 1000 + ";"; //Math.round(reading * 100)
    //alert("created a label z-index:" + Math.round(reading * 100));
    label.position = loc;
    if (unit.indexOf("kg") == -1) {
...
function gm_show_count(count) {
    for (var key in gm_map_labels) {
		gm_map_labels[key].set('text', "Live: " + Math.round(count) + " locations<br/>");
	}
}
